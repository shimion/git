...
function ocmx_theme_update(){
	...
	$i = 2;
	$theme_updates = 0;
	$response = wp_remote_get( "http://www.obox-design.com/hotfixes-".$productid.".xml" ); ?>
	 <div class="rss-widget">
			 <div class="table table_content">
			<h3> Welcome to the OCMX Theme Updater</h3>
			<?php if( is_wp_error( $response ) ){
				echo '<p>It seems that your server is not setup to allow remote queries, and therefore will not allow the update list to load. We suggest <a href="http://www.obox-design.com/hotfixes.cfm?theme=' .$productid . '" target="_blank">downloading the updates manually</a></p>';
				return;
			} else {
		...
		<table class="widefat">
				...
				<tbody>
					<?php if($xml != "") :
						$updates_run_array = get_option("theme-$productid-updates");
